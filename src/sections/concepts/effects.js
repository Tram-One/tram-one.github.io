...
module.exports = (attrs) => {
	return html`
		<section>
			...
			<section-container>
				<section-text>
					Effects are functions that run after elements have been added, updated, or removed. They are inspired by
					<a href="https://reactjs.org/">React's</a> useEffect hook. <br /><br />
			...
			<section-container>
				<section-text>
					If the effect is dependent on a observable object, it will automatically trigger again when that dependent
					property updates.
				</section-text>
				<code-block> ${useEffectWithObservable} </code-block>
			</section-container>
			<section-container>
				<section-text>
					If the effect returns a function, that function will be called when the component is updated or removed.
					<br /><br />
					If the effect does not return a function, the return is ignored, which means you can make the entire effect an
					async function and the returned promise will be ignored.
				</section-text>
				<code-block> ${useEffectWithCleanup} </code-block>
			</section-container>
		</section>
	`;
};
