...
module.exports = (attrs) => {
	return html`
		<section>
			...
			<section-container>
				<section-text>
					Observables are how Tram-One keeps track of state. They are inspired heavily by the
					<a href="https://svelte.dev/">Svelte</a> framework. <br /><br />
					When you update an observable, only the components (and effects) that are dependent on that state are updated.
				</section-text>
				<code-block> ${comboObservable} </code-block>
			</section-container>
			<section-container>
				<section-text> Observables can be tapped into by using the useStore and useGlobalStore hooks. </section-text>
				<code-block> ${globalObservable} </code-block>
			</section-container>
			<section-container>
				<section-text>
				...
				<code-block> ${objectObservable} </code-block>
			</section-container>
		</section>
	`;
};
